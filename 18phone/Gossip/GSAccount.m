...
- (void)setCavDeviceType:(int)type {
    pjsua_acc_config accConfig;
    pjsua_acc_get_config(_accountId, pjsua_pool_create("tmp-pjsua", 1000, 1000), &accConfig);
    if(type == PJMEDIA_VID_DEFAULT_CAPTURE_DEV) {
        accConfig.vid_out_auto_transmit = PJ_TRUE;
    }else if (type == PJMEDIA_VID_DEFAULT_RENDER_DEV) {
        accConfig.vid_out_auto_transmit = PJ_FALSE;
    }
    accConfig.vid_cap_dev = type;
}
